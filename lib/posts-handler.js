  ...
  switch (req.method) {
    case 'GET':
      ...
      Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
        ...
        const oneTimeToken = crypto.randomBytes(8).toString('hex'); // oneTimeTokenをつくる
        oneTimeTokenMap.set(req.user, oneTimeToken); // 連想配列にonTimeTokenを登録
        res.end(pug.renderFile('./views/posts.pug', {
          posts: posts,
          user: req.user,
          oneTimeToken: oneTimeToken //テンプレートにoneTimeTokenというプロパティ名でトークンを渡す
        }));
        console.info(
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        //受け取ったデータを、 & で分割した後、投稿内容とリクエストで送られたワンタイムトークンを取得しています。
        ...
        //また、投稿内容がない場合やワンタイムトークンがない場合には、空文字列を利用するように条件演算子 ? と : を利用して記述しています。
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
          console.info('投稿されました: ' + content);
          //この処理では、連想配列に格納されているワンタイムトークンとリクエストされたワンタイムトークンが同じときである場合のみ、投稿をデータベースに保存する処理を行っています。
          Post.create({
            content: content,
            ...
            //そして投稿が成功した後に、連想配列 oneTimeTokenMap の delete 関数を利用して、利用済みのトークンを連想配列から除去しています
            handleRedirectPosts(req, res);
          });
        } else {
          util.handleBadRequest(req, res);
          //トークンが正しくない場合に、 400 - Bad Request のステータスコードを返します
        }
      });
      break;
    default:
  ...
  switch (req.method) {
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('oneTimeToken=')[1] : '';
        console.info('req.user: ' + req.user); //動作しなかった原因分析用
        console.info('削除対象ID: ' + id); //動作しなかった原因分析用
        console.info('oneTimeTokenMap.get(req.user): ' + oneTimeTokenMap.get(req.user)); //動作しなかった原因分析用
        console.info('requestedOneTimeToken: ' + requestedOneTimeToken); //動作しなかった原因分析用
        if (oneTimeTokenMap.get(req.user) === requestedOneTimeToken) {
          Post.findByPk(id).then((post) => {
            console.info('post.postedBy: ' + post.postedBy); //動作しなかった原因分析用
